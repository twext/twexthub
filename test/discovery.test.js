...
async function approvePending(adminToken, ns, id) {
  ...
  const r = await request(app)
    .patch(`/v0/@${ns}/${id}/versions/${entry.version}`)
    .set(bearer(adminToken))
...
test('search filters by name/id/namespace', async () => {
  ...
  const none = await request(app).get('/v0/search?query=zzzz').expect(200);
...
test('extensions pagination cursor walks all pages', async () => {
  ...
  let guard = 0;
  do {
    const url = '/v0/extensions?limit=2' + (cursor ? `&cursor=${encodeURIComponent(cursor)}` : '');
...
test('stats reports published count, pending, and authors', async () => {
  ...
  let stats = await request(app).get('/v0/stats').expect(200);
  ...
  stats = await request(app).get('/v0/stats').expect(200);
...
test('terms and privacy documents are public', async () => {
  const terms = await request(app).get('/v0/terms').expect(200);
  ...
  const privacy = await request(app).get('/v0/privacy').expect(200);
...
test('terms/accept records acceptance for a session', async () => {
  ...
  const me = await request(app).get('/v0/auth/me').set(bearer(token)).expect(200);
