...
async function publishPending(app, { token, namespace, id, version }) {
  return request(app)
    ...
    .set(bearer(token))
    .send(publishPayload({ id, version }));
}
...
test('a terms bump notifies only accounts that had accepted the previous version', async () => {
  ...
  // The gate is strict: every gated endpoint now demands the new version.
  await request(app).get('/v1/notifications').set(bearer(accepted.token)).expect(403);
  await request(app).get('/v1/notifications').set(bearer(neverAccepted.body.token)).expect(403);

  // Accepting the new version restores access. Only accounts that had
  // accepted the previous version are notified about the bump.
  await request(app).post('/v1/terms/accept').set(bearer(accepted.token)).expect(204);
  await request(app).post('/v1/terms/accept').set(bearer(admin.token)).expect(204);
  await request(app).post('/v1/terms/accept').set(bearer(neverAccepted.body.token)).expect(204);
  ...
  const neverList = await notificationsFor(neverAccepted.body.token);
  assert.equal(neverList.data.length, 0);
});
...
test('the first terms document is not a bump', async () => {
  ...
  // The first document is not a bump, but it re-gates everyone until they
  // accept it.
  await request(app).post('/v1/terms/accept').set(bearer(admin.token)).expect(204);
