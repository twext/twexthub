...
test('create/list/update/delete automation tokens', async () => {
  ...
  const list = await request(app).get('/v0/tokens').set(bearer(token)).expect(200);
  ...
  const updated = await request(app).get('/v0/tokens').set(bearer(token)).expect(200);
...
test('automation token can publish with publish scope', async () => {
  ...
  await request(app)
    .patch(`/v0/@${ns}/hello/versions/${version}`)
    .set(bearer(ab.token))
...
test('automation tokens cannot access session-only endpoints', async () => {
  ...
  const me = await request(app).get('/v0/auth/me').set(bearer(auto)).expect(200);
  assert.equal(me.body.namespace, user.namespace);

  const sessions = await request(app).get('/v0/sessions').set(bearer(auto)).expect(403);
  assert.match(sessions.body.detail, /Automation tokens/i);

  const tokens = await request(app).get('/v0/tokens').set(bearer(auto)).expect(403);
...
test('yank scope: yank own version once approved', async () => {
  ...
  const queue = await request(app)
    ...
    .expect(200);
  await request(app)
    .patch(`/v0/@${ns}/hello/versions/${queue.body.data[0].version}`)
    .set(bearer(ab.token))
    .send({ status: 'approved' })
    .expect(200);
  ...
  await request(app)
    .delete(`/v0/@${ns}/hello/versions/${version}`)
    .set(bearer(yankToken))
    .expect(204);

  const entry = await request(app).get(`/v0/@${ns}/hello/versions/${version}`);
